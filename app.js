document.getElementById("connectWallet").addEventListener("click", async () => {
  ...
  document.getElementById("submitButton").addEventListener("click", async function () {
      ...
      try {
        const labelsData = await handleFileSelect(labelsFile);
        console.log("Labels data:", labelsData);
        labelsArrs.set(modelIdInput, labelsData);
        ...
        try {
            ...
            console.log("FUNC: ", labelsArrs.get(modelIdInput))
        } catch (error) {
  ...
  async function finalizeModelDeletion(modelId, modelList, entry, highestPredictions, highestUploadId) {
    if (highestUploadId) {
        ...
        const testGuessesUrl = prompt("Provide the file URL for your test guesses whose hash was revealed:");
        ...
        if (testGuessesUrl) {
            ...
            if (guessesInfo && guessesInfo[highestUploadId] === hashTestGuessesUrl) {
                alert("Testing labels verified.");
                //const file = await fetchFileAsBlob(testGuessesUrl); // Fetch the file as Blob
                //const labelsData = await handleFileSelect(file); // Process the file to get labels data
                //compareLabelsArrays(labelsArrs.get(modelId), labelsData);
            } else {
...
function compareLabelsArrays(original, newLabels) {
          ...
          if (expirationSpan) {
              ...
              if (expiration < currentTime) {
                ...
                if (userHash === hashToVerify) {
                    ...
                    const success = await displayVerifiedTestLabel(modelId, userHashInput); // Wait for completion
                    if (success) {
        ...
        reader.onload = function(event) {
            ...
            Papa.parse(csvData, {
                complete: function(results) {
                    ...
                    resolve(flatLabels); // Resolve the promise with the processed labels
                },
                error: function(err) {
...
function processLabels(labelsArray) {
  ...
  async function parseAndShuffleData(dataFile, labelsFile) {
      ...
      const trainingLabels = shuffledLabels.slice(0, (shuffledData.length - testingSize));
  ...
  document.getElementById("verifyButton").addEventListener("click", async function () {
      ...
      if (modelHashes.has(modelIdToVerify)) {
          ...
          try {
            ...
            modelTestingGuessesURL.set(modelIdToVerify, { [modelUploadId]: hashGuessesUrl });
            modeluploadResultURL.set(modelIdToVerify, { [modelUploadId]: hashModelUrl });
            console.log("modelTestingGuesses" + modelTestingGuessesURL)
